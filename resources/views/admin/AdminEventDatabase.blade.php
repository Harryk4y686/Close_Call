    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            <div class="bg-white p-8 rounded-xl shadow-sm border border-gray-100 mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-black">Event Database</h1>
                    <p class="text-gray-500 mt-1">Manage your events.</p>
                </div>
                <a href="{{ route('admineventadd') }}" class="bg-[#00A88F] hover:bg-[#008f7a] text-white px-6 py-2.5 rounded-full font-medium transition-colors inline-block">
                    Add Event
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100">
                            <th class="p-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Event Name</th>
                            <th class="p-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Location</th>
                            <th class="p-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Attendees</th>
                            <th class="p-4 text-xs font-medium text-gray-500 uppercase tracking-wider">About</th>
                            <th class="p-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Starting Date</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="p-4 text-sm font-medium text-gray-900">{{ $event->event_name }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $event->location }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $event->attendees }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ Str::limit($event->about, 50) }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $event->starting_date ? $event->starting_date->format('m/d/Y') : 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $event->created_at->format('m/d/Y') }}</td>
                            <td class="p-4 text-center">
                                <a href="{{ route('admineventedit', $event->id) }}" class="w-8 h-8 rounded-full bg-teal-50 text-[#00A88F] flex items-center justify-center hover:bg-teal-100 transition-colors inline-flex">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </a>
                            </td>
                            <td class="p-4 text-center">
                                <form action="{{ route('admineventdelete', $event->id) }}" method="POST" style="display: inline;" class="delete-form">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="8" class="p-8 text-center text-gray-500">
                                No events found. <a href="{{ route('admineventadd') }}" class="text-[#00A88F] hover:underline">Add your first event</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
