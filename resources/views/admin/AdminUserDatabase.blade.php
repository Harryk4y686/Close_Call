    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            <div class="bg-white p-8 rounded-xl shadow-sm border border-gray-100 mb-6 flex justify-between items-center">
                ...
                <a href="{{ route('adminuseradd') }}" class="bg-[#00A88F] hover:bg-[#008f7a] text-white px-6 py-2.5 rounded-full font-medium transition-colors inline-block">
                    Add Users
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="p-4 text-sm font-medium text-gray-900">{{ $user->first_name }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $user->last_name }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $user->email }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $user->phone_number ?? 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->date_of_birth ?? 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->location ?? 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->postal_code ?? 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->resume_path ? 'Yes' : 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->cv_path ? 'Yes' : 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ optional($user->registeredProfile)->portfolio_path ? 'Yes' : 'N/A' }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $user->created_at->format('d/m/Y') }}</td>
                            <td class="p-4 text-center">
                                <a href="{{ route('adminuseredit', $user->id) }}" class="w-8 h-8 rounded-full bg-teal-50 text-[#00A88F] flex items-center justify-center hover:bg-teal-100 transition-colors">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </a>
                            </td>
                            <td class="p-4 text-center">
                                <form action="{{ route('adminuserdelete', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="13" class="p-8 text-center text-gray-500">
                                No users found. <a href="{{ route('adminuseradd') }}" class="text-[#00A88F] hover:underline">Add your first user!</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
